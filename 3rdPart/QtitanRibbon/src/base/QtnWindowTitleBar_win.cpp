    ...
    else
    {
        ...
        HRGN hrgn = region.isNull() ? Q_NULL : 
            qtn_create_region(qtn_toNativeDPI(
                region.translated(QPoint(-frameMargins().left(), -frameMargins().top())), window()));
#else
...
bool WindowTitleBarPrivateWin::handleMessage(MSG* msg, LRESULT& result)
{
    ...
    switch (msg->message)
    {
        ...
        case WM_GETMINMAXINFO:
            {
                ...
                if (msg->message == WM_STYLECHANGED && m_fixupQtNeeded)
        ...
        case WM_SHOWWINDOW:
            {
                const BOOL isVisible = (BOOL)msg->wParam;
                m_glowManager->setVisible(isVisible && ::IsWindowEnabled((HWND)window()->winId()));
...
{
    QRegion regionPart = clipRegion;
    QSize sz = windowSize();
    QRect rect(QPoint(0, 0), sz);
    if (section == Qt::TitleBarArea)
    {
        int h = extendViewIntoTitleBar() ? borderThickness() : titleHeight();
        regionPart -= QRect(QPoint(0, h), QSize(sz.width(), sz.height()));
        rect.setHeight(h);
    }
    else if (section == Qt::LeftSection)
    {
        regionPart -= QRect(QPoint(borderThickness(), 0), QSize(sz.width(), sz.height()));
        rect.setWidth(borderThickness());
    }
    else if (section == Qt::RightSection)
    {
        regionPart -= QRect(QPoint(0, 0), QSize(sz.width() - borderThickness(), sz.height()));
        rect.setWidth(borderThickness());
    }
    else if (section == Qt::BottomSection)
    {
        regionPart -= QRect(QPoint(0, 0), QSize(sz.width(), sz.height() - borderThickness()));
        rect.setHeight(borderThickness());
    }
    else if (section == Qt::NoSection)
    {
        regionPart = QRect(QPoint(0, 0), QSize(sz.width(), sz.height()));
    }

    rect.setSize(qtn_toNativeDPI(rect.size(), window()));
    if (rect.isEmpty())
        return;

    QRegion clipRegionPart = qtn_toNativeDPI(regionPart, window());
    ...
    for (QRegion::const_iterator it = clipRegionPart.begin(); it != clipRegionPart.end(); ++it)
#else
...
void WindowTitleBarPrivateWin::paintNonClientArea(HDC hdc)
{
    ...
    if (m_fillClientArea)
    {
        if (styledFrame())
        {
            QRect rect = qtn_toNativeDPI(getClientRect(), window());
            ImageHDC ncpaintptr(rect.width(), rect.height(), ImageHDC::systemFormat());
#if (QT_VERSION >= QT_VERSION_CHECK(5, 0, 0))
            ncpaintptr.image().setDevicePixelRatio(qtn_pixelRatioDPI(window()));
#endif
