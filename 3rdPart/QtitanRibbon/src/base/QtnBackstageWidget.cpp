...
void BackstageWidget::send_mouseMoveEvent(QMouseEvent* e)
{
    ...
    if (w != Q_NULL)
    {
        ...
        MouseEventQt mouseEvent(*e);
        mouseEvent.setPos(e->pos() - offset);
...
void BackstageWidget::send_wheelEvent(QWheelEvent* e)
{
    ...
    WheelEventQt wheelEvent(*e);
    wheelEvent.accept();
    wheelEvent.setPos(e->pos() - offset);
#if 0 //MACOSX bug
...
void BackstageWidget::send_keyPressEvent(QKeyEvent* e)
{
    if (m_widget == Q_NULL || !m_widget->isEnabled())
    {
        e->ignore();
