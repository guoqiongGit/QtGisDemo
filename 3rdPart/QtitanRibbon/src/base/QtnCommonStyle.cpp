...
void QTITAN_PREPEND_NAMESPACE(qtn_clickLight)(QWidget* widget)
{
    ClickLightAnimation::startClickAnimation(widget);
}

void QTITAN_PREPEND_NAMESPACE(qtn_paintLight)(QWidget* widget, QPainter *p, const QRect& r)
{
    const QColor light = QColor::fromRgba(0x50FFFFFF);

    p->save();
    p->setRenderHint(QPainter::Antialiasing, true);

    QPoint pos = widget->mapFromGlobal(QCursor::pos());
    qreal radius = qMax(r.width() / 2, r.height() / 2) * 2;
    QRadialGradient g(pos /* rect().center() */, radius, pos);
...
void QTITAN_PREPEND_NAMESPACE(qtn_paintLightFrame)(QWidget* widget, QPainter *p, const QRect& r)
{
    const QColor light = QColor::fromRgba(0x90FFFFFF);
    p->save();
    p->setRenderHint(QPainter::Antialiasing, true);
    QPoint pos = widget->mapFromGlobal(QCursor::pos());
...
QPalette StyleConfig::getWidgetPalette(const QString& className) const
{
    ...
    WidgetDataHash::const_iterator data = m_hashWidgetsData.constFind(className);
    if (data == m_hashWidgetsData.constEnd())
        return pal;

    PaletteHash hash = (*data).m_hashPalette;
    for (uint group = QPalette::Active; group <= QPalette::All; ++group)
    {
        if (!hash.contains(static_cast<QPalette::ColorGroup>(group)))
            continue;

        QHash<QPalette::ColorRole, QBrush> hashGroup = hash[static_cast<QPalette::ColorGroup>(group)];
        for (uint role = QPalette::WindowText; role <= QPalette::NColorRoles; ++role)
        {
            if (!hashGroup.contains(static_cast<QPalette::ColorRole>(role)))
                continue;
            QBrush brush = hashGroup[static_cast<QPalette::ColorRole>(role)];
            pal.setBrush(static_cast<QPalette::ColorGroup>(group), static_cast<QPalette::ColorRole>(role), brush);
        }
    }
    return pal;
}
...
QPixmap StandardSymbolIcon::standardSymbolPixmap(QStyle::StandardPixmap sp, const QStyleOption* option, const QWidget* widget) const
{
    ...
    return SegoeMDL2Font::pixmap(symbolStandardPixmap(sp), option ? option->rect.size() : QtnPrivate::qtn_bestImageSize(widget), symbolColor);
}
...
QPalette CommonStylePrivate::getWidgetPalette(QWidget* widget) const
{
    ...
    QPalette pal;
    if (widget->inherits(QTITAN_META_CLASSNAME("Grid")))
        pal = m_config.getWidgetPalette(QStringLiteral("QTableView"));
    else
        pal = m_config.getWidgetPalette(widget);
    
    
    if (isQtitanStyle())
        return pal;

    if (widget->inherits(QTITAN_META_CLASSNAME("RibbonTitleBarWidget")))
    {
        qtn_initTitleBarPalette(pal);
    }
    else if (widget->inherits(QTITAN_META_CLASSNAME("RibbonBar")))
    {
        pal.setBrush(QPalette::Active, QPalette::WindowText, pal.color(QPalette::Dark).darker(240));
        pal.setBrush(QPalette::Inactive, QPalette::WindowText, pal.color(QPalette::Dark).darker(240));
        pal.setBrush(QPalette::Active, QPalette::ButtonText, pal.color(QPalette::Dark).darker(240));
        pal.setBrush(QPalette::Inactive, QPalette::ButtonText, pal.color(QPalette::Dark).darker(240));
#if (QT_VERSION < QT_VERSION_CHECK(5, 0, 0))
        pal.setBrush(QPalette::All, QPalette::Button, p.CommonStyle::standardPalette().button());
#endif
    }
    else if (widget->inherits(QTITAN_META_CLASSNAME("RibbonTabBar")))
    {
        pal.setBrush(QPalette::Active, QPalette::HighlightedText, pal.color(QPalette::ButtonText));
    }
    else if (widget->inherits(QTITAN_META_CLASSNAME("OfficePopupMenu")))
    {
        pal.setBrush(QPalette::All, QPalette::Window, darkBackgroundLight(pal));
        pal.setBrush(QPalette::All, QPalette::Button, darkBackgroundLight(pal));
    }
    else if (widget->inherits(QTITAN_META_CLASSNAME("DockTitleBar")))
    {
        pal.setBrush(QPalette::Active, QPalette::Window, pal.highlight().color().lighter());
        pal.setBrush(QPalette::Inactive, QPalette::Window, p.standardPalette().window());
    }
    else if (widget->inherits(QTITAN_META_CLASSNAME("DockWidgetTitleButton")))
    {
        pal.setBrush(QPalette::All, QPalette::HighlightedText, pal.buttonText().color());
    }
    else if (widget->inherits(QTITAN_META_CLASSNAME("DockWidgetMarkerWidget")))
    {
        pal.setBrush(QPalette::Active, QPalette::Mid, pal.highlight());
        pal.setBrush(QPalette::Active, QPalette::Highlight, pal.highlight());
        pal.setBrush(QPalette::Inactive, QPalette::Mid, QColor(StyleHelper::colorMakeDarker(pal.window().color().rgb(), 0.5)));
        pal.setBrush(QPalette::Inactive, QPalette::Highlight, QColor(StyleHelper::colorMakeDarker(pal.window().color().rgb(), 0.5)));
    }
    else if (widget->inherits(QTITAN_META_CLASSNAME("DockWidgetPhantomWidget")))
    {
        pal.setBrush(QPalette::Active, QPalette::Highlight, QColor(StyleHelper::colorMakeLighter(pal.highlight().color().rgb(), 0.85)));
    }
    return pal;
}
...
/*! \internal */
void CommonStylePrivate::beginRefresh(bool polishNeeded)
{
    QTN_P(CommonStyle);
    if (!polishNeeded)
        return;
    ...
    {
        QWidget* w = *it;
        if (!(w->windowType() == Qt::Desktop) && w->testAttribute(Qt::WA_WState_Polished) && /*w->style()*/qtn_getStyle(w->style()) == &p)
            p.unpolish(w);
    }
}

/*! \internal */
void CommonStylePrivate::endRefresh(bool polishNeeded)
{
    QTN_P(CommonStyle);
    ...
    if (style == &p)
    {
        ...
        for (int role = 0; role < (int)QPalette::NColorRoles; role++)
            resolve_mask |= (1 << role);
        m_systemPalette.resolve(resolve_mask);
...
/*! \internal */
bool CommonStylePrivate::drawIndicatorTabClose(const QStyleOption* /*opt*/, QPainter* /*p*/, const QWidget* /*w*/) const
{ return false; }
...
/*! \internal */
bool CommonStylePrivate::drawIndicatorBranch(const QStyleOption* /*opt*/, QPainter* /*p*/, const QWidget* /*w*/) const
{ return false; }
...
{
    QTN_P(const CommonStyle);
    if (w == Q_NULL || !w->inherits(QTITAN_META_CLASSNAME("Grid")))
        return false;
...
void CommonStylePrivate::drawfillTitleBar(const QStyleOption* option, QPainter* painter, const QWidget* widget, const QColor& frameColor, const QColor& windowColor) const
{
    ...
    if (const QStyleOptionTitleBar* tb = qstyleoption_cast<const QStyleOptionTitleBar *>(option))
    {
        ...
        else if (windowColor != Qt::transparent)
        {
        ...
        if (frameColor.isValid())
        {
            ...
            // draw left part
            QRect rect = QRect(option->rect.x(), option->rect.y() + fwidth, fwidth, option->rect.height() - fwidth);
            painter->fillRect(rect, frameColor);
            // draw right part
...
bool CommonStylePrivate::drawTitleBar(const QStyleOptionComplex* option, QPainter* painter, const QWidget* widget) const
{
    ...
    if (const QStyleOptionTitleBar* tb = qstyleoption_cast<const QStyleOptionTitleBar *>(option))
    {
        ...
        if (tb->subControls & QStyle::SC_TitleBarCloseButton && tb->titleBarFlags & Qt::WindowSystemMenuHint)
        {
            ...
            if (closeHint)
            {
                tool.palette.setColor(QPalette::Active, QPalette::WindowText, Qt::white);
            ...
            {
                tool.palette.setColor(QPalette::Active, QPalette::WindowText, Qt::gray);
                tool.palette.setColor(QPalette::Active, QPalette::HighlightedText, Qt::gray);
            }
        ...
        if (tb->subControls & QStyle::SC_TitleBarMaxButton && tb->titleBarFlags & Qt::WindowMaximizeButtonHint && !(tb->titleBarState & Qt::WindowMaximized)) 
        {
            ...
            tool.palette.setColor(QPalette::Active, QPalette::Highlight, tb->palette.color(QPalette::Active, QPalette::Highlight).darker(120));
            p.proxy()->drawPrimitive(QStyle::PE_PanelButtonTool, &tool, painter, widget);
...
/*! \internal */
bool CommonStylePrivate::drawTabBarTabLabel(const QStyleOption* /*opt*/, QPainter* /*painter*/, const QWidget* /*w*/) const
{
    return false;
}
...
bool CommonStylePrivate::drawToolBar(const QStyleOption* /*opt*/, QPainter* /*painter*/, const QWidget* /*w*/) const
{
    return false;
}
...
/*! \internal */
bool CommonStylePrivate::drawTabBarTabShape(const QStyleOption* option, QPainter* painter, const QWidget* w) const
{
    if (w && w->inherits(QTITAN_META_CLASSNAME("DockAutoHideLayoutPanel")))
    {
...
{
    if (w && w->inherits(QTITAN_META_CLASSNAME("Grid")))
    {
#if 0
...
#if (QT_VERSION >= QT_VERSION_CHECK(5, 0, 0))
            QPixmap pixmap = !option->icon.isNull() ? option->icon.pixmap(QtnPrivate::qtn_getWindow(w), QSize(iconSize, iconSize),
                (option->state & QStyle::State_Enabled) ? QIcon::Normal : QIcon::Disabled) : QPixmap();
#else
            QPixmap pixmap = !option->icon.isNull() ? option->icon.pixmap(QSize(iconSize, iconSize), 
                (option->state & QStyle::State_Enabled) ? QIcon::Normal : QIcon::Disabled) : QPixmap();
#endif
            QRect textRect = qtn_paint_icon_and_text(*painter, option->direction, palette,
                rect, pixmap, option->iconAlignment, option->text, option->textAlignment, option->textElideMode, margin);
...
bool CommonStylePrivate::drawDockWidgetTitle(const QStyleOption* option, QPainter* painter, const QWidget* w) const
{
    ...
    if (w && w->inherits(QTITAN_META_CLASSNAME("DockTitleBar")))
    {
        if (const DockTitleBarStyleOption* dwOpt = qstyleoption_cast<const DockTitleBarStyleOption*>(option))
        {
            ...
            painter->drawRect(rc.adjusted(0, 1, -1, -3));
...
/*! \internal */
bool CommonStylePrivate::drawRibbonBar(const QStyleOption* opt, QPainter* p, const QWidget* widget) const
{
    Q_UNUSED(widget)
    if (const RibbonStyleOption* optRibbon = qstyleoption_cast<const RibbonStyleOption*>(opt))
    {
        ...
        if (optRibbon->minimized)
        {
            ...
            QPen savePen = p->pen();
            p->setPen(optRibbon->palette.dark().color().lighter(135));
            p->drawPath(path);
            p->setPen(savePen);
        }
        return true;
...
{
    Q_UNUSED(widget)
    if (const RibbonStyleOption* optGroups = qstyleoption_cast<const RibbonStyleOption*>(option))
    {
        ...
        if (optGroups->tabBarPosition == 1)
        {
            ...
            QPen savePen = p->pen();
            QColor color = CommonStyle::qtn_getColorToRGB(optGroups->pageColor);
        ...
        else if (optGroups->tabBarPosition == 2)
        {
            QPainterPath path;
...
bool CommonStylePrivate::drawRibbonSysFramePagePopupList(const QStyleOption* opt, QPainter* painter, const QWidget* widget) const
{
    ...
    pal.setColor(QPalette::WindowText, pal.color(QPalette::Dark).darker(150));
    ...
    if (const QStyleOptionToolButton* toolbutton = qstyleoption_cast<const QStyleOptionToolButton*>(opt))
    {
        ...
        if (mouseInSplit || checked)
        {
            ...
            if (colorSplitFill.isValid())
            {
                ...
                drawRectDPI(*painter, buttonRectUp);
            }
        }
        if ((mouseInSplitDropDown || popuped || checked) && enabled)
        {
            if (!popuped)
            {
                ...
                QtnPen pen(painter, buttonHighlightColor(opt->palette, pressed, true));
                drawRectDPI(*painter, popupRectDown);
            }
...
/*! \internal */
bool CommonStylePrivate::drawRibbonImageBackground(const QStyleOption* /*opt*/, QPainter* /*painter*/, const QWidget* /*w*/) const
{
    return false;
}
...
/*! \internal */
bool CommonStylePrivate::drawRibbonTab(const QStyleOption* opt, QPainter* p, const QWidget* widget) const
{
    Q_UNUSED(widget)
    if (const RibbonOptionHeaderStyleOption* optTab = qstyleoption_cast<const RibbonOptionHeaderStyleOption*>(opt))
    {
        ...
        if ((optTab->state & QStyle::State_Selected) || isHighlight)
        {
            QRect rectTab = optTab->rect.adjusted(0, 0, 0, -1);
            // fill the background of the tab
            p->fillRect(rectTab, !(optTab->state & QStyle::State_Selected) && isHighlight ? optTab->palette.mid().color().lighter(158) : optTab->palette.button().color());
            QPainterPath path;
            ...
            p->setPen(colorContext.isValid() ? colorContext : (isHighlight ? optTab->palette.midlight().color() : optTab->palette.midlight().color()));
            p->drawPath(path);
            p->setPen(savePen);
        }
        else if (optTab->state & QStyle::State_Small)
        {
            ...
            path.lineTo(rectTab.right(), rectTab.bottom());
            QPen savePen = p->pen();
            p->setPen(optTab->palette.dark().color().lighter(135));
            p->drawPath(path);
            p->setPen(savePen);
        }
        else
        {
            if (optTab->pageColor != Qtitan::ContextColorNone && optTab->firstTab)
            {
                ...
                gradient.setColorAt(0, optTab->palette.dark().color().lighter(110));
            ...
            if (optTab->pageColor != Qtitan::ContextColorNone && optTab->lastTab)
            {
...
/*! \internal */
bool CommonStylePrivate::drawRibbonTabBar(const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    Q_UNUSED(opt)
    Q_UNUSED(p)
    Q_UNUSED(w)
...
bool CommonStylePrivate::drawRibbonSysMenuItem(const QStyleOption* opt, QPainter* painter, const QWidget* widget) const
{
    ...
    if (const StyleOptionMenuScrollItem* menuitem = qstyleoption_cast<const StyleOptionMenuScrollItem *>(opt))
    {
        ...
        if (selected)
        {
            ...
            QRect rect = menuitem->rect.adjusted(0, 0, -1, -1);
...
bool CommonStylePrivate::drawRibbonButtonLabel(const QStyleOption* opt, QPainter* painter, const QWidget* w) const
{
    ...
    if (const QStyleOptionToolButton* toolbutton = qstyleoption_cast<const QStyleOptionToolButton*>(opt))
    {
        ...
        if (isAncestorTabBar && isQtitanStyle())
        {
            ...
            palText.setBrush(QPalette::ButtonText, clrText);
            ...
            palText.setBrush(QPalette::HighlightedText, clrText);
        }
...
bool CommonStylePrivate::drawRibbonGalleryItem(const QStyleOption* opt, QPainter* painter, const QWidget* w) const
{
    ...
    if (const RibbonGalleryItemStyleOption* optItem = qstyleoption_cast<const RibbonGalleryItemStyleOption*>(opt))
    {
        ...
        if (optItem->separator)
        {
            ...
            pal.setColor(QPalette::WindowText, colText);
...
bool CommonStylePrivate::drawRibbonReducedGroup(const QStyleOption* opt, QPainter* painter, const QWidget* w) const
{
    ...
    if (const QStyleOptionToolButton* optGroup = qstyleoption_cast<const QStyleOptionToolButton*>(opt))
    {
        ...
        optButton.state = opt->state;
        optButton.palette = optGroup->palette;
        if (optGroup->features & QStyleOptionToolButton::HasMenu)
            optButton.features |= QStyleOptionButton::HasMenu;
...
bool CommonStylePrivate::drawNavigationBackButton(const QStyleOption* opt, QPainter* painter, const QWidget* w) const
{
    ...
    temp.palette.setColor(QPalette::Button, Qt::transparent);
    temp.palette.setColor(QPalette::Highlight, opt->palette.color(QPalette::Highlight).darker(120));
...
bool CommonStylePrivate::drawRibbonFileButton(const QStyleOptionComplex* opt, QPainter* painter, const QWidget* widget) const
{
    ...
    if (const SystemToolButtonStyleOption* optButton = qstyleoption_cast<const SystemToolButtonStyleOption*>(opt))
    {
        ...
        uint resolve_mask = optButton->palette.resolve();
    ...
    if (val == CommonStylePrivate::InvalidMetric)
    {
        switch (static_cast<CommonStyle::PixelMetricEx>(metric))
        {
            ...
            case CommonStyle::PM_RibbonHeightCaptionGroup:
                ...
                break;
            case CommonStyle::PM_RibbonReducedGroupWidth:
...
/*! \internal */
void CommonStylePrivate::drawRectDPI(QPainter& painter, const QRect& rect) const
{
...
/*! \internal */
void CommonStylePrivate::drawLineDPI(QPainter& painter, const QRect& rect, bool left, bool top, bool right, bool bottom) const
{
//    QPaintDevice* device = painter.device();
    qreal factor = 1.0;// device->devicePixelRatioF();
    qreal penWidth = painter.pen().widthF() * factor;
...
QString CommonStylePrivate::toolButtonElideText(const QStyleOptionToolButton *option, const QRect &textRect, int flags) const
{
    ...
    if (option->fontMetrics.width(option->text) <= textRect.width())
        return option->text;
#else
    if (option->fontMetrics.boundingRect(option->text).width() <= textRect.width())
        return option->text;
#endif
...
void CommonStyle::unpolish(QApplication* app)
{
    ...
    QTN_D(CommonStyle);
    d.m_standardPalette = QPalette();
    d.m_systemPalette = QPalette();
    QApplication::setPalette(QPalette());
...
void CommonStyle::unpolish(QWidget* widget)
{
    ...
    if (original_palette.resolve() != 0)
    {
        QPalette palette = widget->palette();
        int mask = palette.resolve();
        {
            for (int role = 0; role < (int)QPalette::NColorRoles; ++role)
                for (int group = 0; group < (int)QPalette::NColorGroups; ++group)
...
void CommonStyle::drawPrimitive(QStyle::PrimitiveElement pe, const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    ...
    switch (static_cast<CommonStyle::PrimitiveElementEx>(pe))
    {
        ...
        case CommonStyle::PE_RibbonImageBackground:      draw = d.drawRibbonImageBackground(opt, p, w); break;
...
int CommonStyle::pixelMetric(QStyle::PixelMetric metric, const QStyleOption* opt, const QWidget* widget) const
{
    ...
    if (metric == PM_TitleBarHeight && widget != Q_NULL && widget->inherits(QTITAN_META_CLASSNAME("DockWindow")))
    ...
    switch (metric)
    {
        ...
        case PM_TabBarTabHSpace:
            if (widget && widget->inherits(QTITAN_META_CLASSNAME("DockDocumentAreaTabBar")))
        ...
        case PM_ToolBarHandleExtent:
            val = d.m_config.pixelMetric(widget, className.isEmpty() ? QStringLiteral("QToolBar") : className, QStringLiteral("ToolBarHandleExtent"), defaultValue, &ok);
            break;
        case PM_ToolBarItemSpacing:
            val = d.m_config.pixelMetric(widget, className.isEmpty() ? QStringLiteral("QToolBar") : className, QStringLiteral("ToolBarItemMargin"), defaultValue, &ok);
            break;
        case PM_ToolBarFrameWidth:
            val = d.m_config.pixelMetric(widget, className.isEmpty() ? QStringLiteral("QToolBar") : className, QStringLiteral("ToolBarFrameWidth"), defaultValue, &ok);
            break;
...
QRgb CommonStyle::qtn_lightColor(QRgb clr, int nTint)
{
    return StyleHelper::lightColor(clr, nTint);
}

QRgb CommonStyle::qtn_darkColor(QRgb clr, int nShade)
{
    return StyleHelper::darkColor(clr, nShade);
}

QColor CommonStyle::qtn_getColorToRGB(Qtitan::ContextColor color)
{
    switch (color)
    {
...
QString CommonStyle::qtn_getColorToString(Qtitan::ContextColor color)
{
    switch (color)
    {
        case Qtitan::ContextColorBlue:
...
QImage CommonStyle::qtn_addaptColors(const QImage& imageSrc, QRgb clrBase, QRgb clrTone, bool clampHue)
{
    return StyleHelper::addaptColors(imageSrc, clrBase, clrTone, clampHue);
}

QImage CommonStyle::qtn_invertColors(const QImage& pxSrc)
{
...
QRect QTITAN_PREPEND_NAMESPACE(qtn_paint_icon_and_text)(QPainter& painter, Qt::LayoutDirection direction, const QPalette& palette, const QRect& r,
    const QPixmap& pixmap, Qt::Alignment iconAlignment, const QString& text, Qt::Alignment textAlignment, Qt::TextElideMode textElideMode,
...
#endif
        const qreal w = static_cast<qreal>(pixmap.width()) / pixelRatio;
        QRect aligned = QStyle::alignedRect(direction, iconAlignment, pixmap.size() / pixelRatio, rect);
        QRect intersect = aligned.intersected(rect);

        painter.drawPixmap(intersect.x(), intersect.y(), pixmap,
            intersect.x() - aligned.x(), intersect.y() - aligned.y(),
            qRound(static_cast<qreal>(aligned.width()) * pixelRatio),
            qRound(static_cast<qreal>(pixmap.height()) * pixelRatio));

        if (direction == Qt::LeftToRight)
            rect.setLeft(rect.left() + static_cast<int>(w) + margin);
        else
            rect.setRight(rect.right() - static_cast<int>(w) - margin);
    }
...
void QTITAN_PREPEND_NAMESPACE(qtn_paint_keyboard_focus)(QPainter* painter, const QRect& rect, const QColor& color)
{
    ...
    painter->setPen(pen);
    painter->drawRect(r.adjusted(thickness / 2, thickness / 2, -((thickness + 1) / 2), -((thickness + 1) / 2)));
}
