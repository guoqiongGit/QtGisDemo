...
\class ToolTip
\inmodule QtitanBase
\brief The ToolTip class provides tool tips (balloon help) for any widget.
*/
/*!
...
/*!
\brief Destructor of the ToolTip object.
*/ 
ToolTip::~ToolTip()
{
...
*/ 
void ToolTip::showToolTip(const QPoint &pos, const QString& title, const QString& text, const QIcon& icon, QWidget* w)
{
    QRect rect;
    ...
    if (ToolTipPrivate::m_instance && ToolTipPrivate::m_instance->isVisible())
    {
        ...
        else if (!ToolTipPrivate::m_instance->fadingOut())
        {
            ...
            if (ToolTipPrivate::m_instance->tipChanged(localPos, text, title, w))
            {
                ToolTipPrivate::m_instance->setIcon(isIconVisible() ? icon : QIcon());
                ToolTipPrivate::m_instance->reuseTip(title, text);
                ToolTipPrivate::m_instance->setTipRect(w, rect);
                ToolTipPrivate::m_instance->placeTip(pos, w);
            }
            return;
        }
    }
    ...
    if (!text.isEmpty())
    {
        new ToolTip(title, text, icon, w); // sets TipLabel::m_pInstanceto itself
        ToolTipPrivate::m_instance->setTipRect(w, rect);
        ToolTipPrivate::m_instance->placeTip(pos, w);
