...
{
public:
    WidgetData() : m_hashPalette(), m_subControlHash(),  hover(-1), autoFillBackground(-1){}
    WidgetData(const WidgetData& other) : m_hashPalette(other.m_hashPalette), 
        m_subControlHash(other.m_subControlHash), hover(other.hover), autoFillBackground(other.autoFillBackground){}
public:
...
public: // useful functions
    ...
    void beginRefresh(bool polishNeeded = true);
    void endRefresh(bool polishNeeded = true);
public:
...
protected: // additional rendering functions
    ...
    void drawRectDPI(QPainter& painter, const QRect& rect) const;
    void drawLineDPI(QPainter& painter, const QRect& rect, bool left, bool top, bool right, bool bottom) const;
    ...
    virtual void drawfillTitleBar(const QStyleOption* option, QPainter* painter, const QWidget* widget, const QColor& frameColor, const QColor& windowColor) const;
protected:
    /*! \internal */
    enum TypePaint
    {
...
QTITAN_BASE_EXPORT void qtn_paintLight(QWidget* widget, QPainter *p, const QRect& r);
QTITAN_BASE_EXPORT void qtn_paintLightFrame(QWidget* widget, QPainter *p, const QRect& r);
QTITAN_BASE_EXPORT void qtn_clickLight(QWidget* widget);

namespace QtnPrivate
{
    inline bool qtn_isExistWindowTitleBar(const QWidget* widget)
    {
    ...
    inline QPixmap qtn_Pixmap(const QIcon& icon, const QRect& rect, const QSize& iconSize, QStyle::State state, const QWidget* widget)
    {
        if (!icon.isNull())
        {
