    ...
    m_buffer(0, 0, QImage::Format_ARGB32_Premultiplied)
{
    ...
    {
        qWarning("Warning: Qtitan::MaterialWidget does not support Qt::AA_EnableHighDpiScaling.");
    }
#endif
...
void MaterialWidgetEffect::draw(QPainter *painter)
{
    ...
    if (fullyUpdate)
    {
...
#else
        if (m_blurredBuffer.size() != buffer.size() || m_blurredBuffer.format() != buffer.format())
        ...
        m_blurredBuffer.fill(Qt::transparent);
        {
            ...
            p.drawImage(QRect(QPoint(0, 0), buffer.size() / buffer.devicePixelRatio()), buffer);
#else
