...
{
    Q_ASSERT(window != Q_NULL && window->isTopLevel());
    ...
    void checkMouseButtons()
    {
        if (m_init)
        {
            Qt::MouseButtons buttons =
                    ...
                    qtn_get_mouseButtons_linux();
#elif defined (Q_OS_MAC)
                    qtn_get_mouseButtons_mac();
#else
...
bool WindowTitleBarPrivate::mouseEventFilter(QObject* object, QEvent* event)
{
...
#endif
    {
...
#endif
        {
            ...
            me.setPos(tranlateToNonClient(me.pos()));
#else
...
bool WindowTitleBarPrivate::eventFilter(QObject* object, QEvent* event)
{
    if (object == window())
    {
        ...
        case QEvent::KeyPress:
        {
            if (m_titleBarWidget != Q_NULL)
            {
                m_titleBarWidget->send_keyPressEvent(static_cast<QKeyEvent *>(event));
                if (event->isAccepted())
                    return true;
           }
        }
...
void WindowTitleBarPrivate::setTitleBarWidget(QWidget* widget)
{
    ...
    {
        if (m_titleBarWidget != Q_NULL)
            m_titleBarWidget->setWidget(Q_NULL, QRect());
        Q_DELETE_AND_NULL(m_titleBarWidget);
        return;
    }
...
{
    if (window() != Q_NULL)
        window()->update(rect.adjusted(-1, -1, 1, 1));
}
...
Qt::WindowFrameSection WindowTitleBarPrivate::checkFrameSection(const QPoint& pos)
{
    ...
    const int sysBorder = qMax(systemFrameMargins().left(), 5);
...
void WindowTitleBarPrivate::initTitleBarOption(QStyleOptionTitleBar& option)
{
    ...
    if (extendViewIntoTitleBar())
    {
        ...
        option.palette.setColor(QPalette::Shadow, m_titleBarPalette.color(
            isActive() ? QPalette::Active : QPalette::Inactive, QPalette::Shadow));
    }
    else
    ...
    if (hasCustomBackground())
    {
        ...
        if (isActive())
            option.palette.setColor(QPalette::Shadow, backgroundColor());
        else
            option.palette.setColor(QPalette::Shadow, backgroundColor().darker(150));
    }
...
void WindowTitleBarPrivate::paintTitleBarText(QPainter* painter, const QStyleOptionTitleBar& option)
{
    ...
    QColor textColor = option.palette.color(isActive() ? QPalette::Active : QPalette::Inactive, QPalette::WindowText);
    if (!styledFrame())
    {
        qtn_paintTitleBarText(painter, option.text, option.rect, isActive(), textColor);
    }
    else
    {
...
{
    if (m_extendedView != Q_NULL)
        m_extendedView->setGeometry(QRect(QPoint(-frameMargins().left(), -frameMargins().top()), windowSize()));
}
...
void ExendedView::paintEvent(QPaintEvent *event)
{
    ...
    QRegion clip = m_bar->getNonClientRegion().intersected(event->region());
    painter.setClipRegion(clip);
    if (!clip.isEmpty())
