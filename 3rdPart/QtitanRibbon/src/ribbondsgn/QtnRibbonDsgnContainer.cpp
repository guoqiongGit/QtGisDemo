    ...
    : QObject(pWidget)
{
    ...
    m_ribbonTabBar->setObjectName(QStringLiteral("__qt__passive_RibbonTabBar"));
}
...
static void setObjectNameChildren(RibbonTabBar& widget)
{
    ...
    for(int i = 0, count = child_list.count(); count > i; i++)
    {
        if (RibbonTab* pWd = child_list[i])
            pWd->setObjectName(QStringLiteral("__qt__passive_RibbonTab"));
    }
}
