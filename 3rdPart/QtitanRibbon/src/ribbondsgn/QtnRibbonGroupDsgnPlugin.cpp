...
QWidget* DsgnRibbonGroupPlugin::createWidget(QWidget* parent)
{
    ...
    RibbonGroup* result = new RibbonGroup(parent);
    connect(result, SIGNAL(titleChanged(QString)), this, SLOT(titleChanged(QString)));
    result->setObjectName(QStringLiteral("__qt__passive_RibbonGroup"));
    result->setAcceptDrops(true);
    return result;
...
{
    return create_qtitan_className(QStringLiteral("RibbonGroup"));
}
...
{
    return tr("Advanced RibbonPage for Qt++");
}
...
{
    QString className = create_qtitan_className(QStringLiteral("RibbonGroup"));
    return QStringLiteral("<ui language=\"c++\">\n"
        " <widget class=\"" "%1" "\" name=\"ribbonGroup\"/>\n"
        " <customwidgets>\n"
        "   <customwidget>\n"
        "       <class>" "%1" "</class>\n"
...
void DsgnRibbonGroupPlugin::titleChanged(const QString& title)
{
    ...
    if (RibbonGroup* ribbonGroup = qobject_cast<RibbonGroup*>(sender())) 
    {
        if (QDesignerFormWindowInterface* form = QDesignerFormWindowInterface::findFormWindow(ribbonGroup))
        {
            ...
            const int propertyIndex = sheet->indexOf(QStringLiteral("title"));
...
{
    m_ribbonPage->detachGroupByIndex(index);
}
...
{
    Q_ASSERT(m_ribbonPage != Q_NULL);
    return m_ribbonPage->groupAt(index);
}
