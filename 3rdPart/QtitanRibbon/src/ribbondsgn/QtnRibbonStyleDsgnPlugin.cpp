...
void DsgnRibbonStyleWidget::paintEvent(QPaintEvent* event)
{
    ...
    painter.drawText(area, Qt::AlignCenter, QStringLiteral("Ribbon Style"));
}
...
void DsgnOfficeStyleWidget::paintEvent(QPaintEvent *)
{
    ...
    painter.setFont(f);
    QRect area = QRect(1, 1, height() - 1, width() - 1);
    painter.drawText(area, Qt::AlignCenter, QStringLiteral("Microsoft Office Style"));
}
...
QWidget* QTITAN_PREPEND_NAMESPACE(create_fake_ribbon_style_widget)(QWidget* parent)
{
...
QWidget* QTITAN_PREPEND_NAMESPACE(create_fake_office_style_widget)(QWidget* parent)
{
...
QObject* ExtensionManagerWrapper::extension(QObject *object, const QString &iid) const
{
    ...
    if (m_nativeManager != Q_NULL)
    {
        if (object != Q_NULL && QLatin1String(object->metaObject()->className()) == create_qtitan_className(QStringLiteral("RibbonStyle")))
            object = static_cast<DsgnRibbonStyleWidget *>(object)->targetStyle();
        if (object != Q_NULL && QLatin1String(object->metaObject()->className()) == create_qtitan_className(QStringLiteral("OfficeStyle")))
...
{
    if (QLatin1String(widget->metaObject()->className()) == create_qtitan_className(QStringLiteral("RibbonStyle")))
    {
    ...
    else if (QLatin1String(widget->metaObject()->className()) == create_qtitan_className(QStringLiteral("OfficeStyle")))
    {
        if (m_styleWidget != Q_NULL)
            remove(count() - 1);
        Q_ASSERT(m_styleWidget == Q_NULL);
...
{
    if (QLatin1String(widget->metaObject()->className()) == create_qtitan_className(QStringLiteral("RibbonStyle")) ||
        QLatin1String(widget->metaObject()->className()) == create_qtitan_className(QStringLiteral("OfficeStyle")))
    {
        addWidget(widget);
        return;
    }
...
{
    return QIcon(QStringLiteral(":res/style_icon.png"));
}
...
{
    return QStringLiteral("Developer Machines: Styles");
}
...
{
    return create_qtitan_className(QStringLiteral("OfficeStyle"));
}
...
void OfficeStyleDsgnPlugin::widgetManaged(QWidget* widget)
{
    ...
    if (QLatin1String(widget->metaObject()->className()) == create_qtitan_className(QStringLiteral("OfficeStyle")))
    {
        ...
        for (int i = 0; i < container->count() && widgetStyle == Q_NULL; ++i)
        {
            ...
            if (QLatin1String(w->metaObject()->className()) == create_qtitan_className(QStringLiteral("OfficeStyle")) ||
                QLatin1String(w->metaObject()->className()) == create_qtitan_className(QStringLiteral("RibbonStyle")))
                widgetStyle = w;
        }

        if (widgetStyle && QLatin1String(widgetStyle->metaObject()->className()) == create_qtitan_className(QStringLiteral("OfficeStyle")))
        {
...
*/
    ...
    else if (DesignerMainWindowStyleContainer* styelContainer = dynamic_cast<DesignerMainWindowStyleContainer*>(container))
    {
        ...
        {
            if (OfficeStyle* currentStyle = qobject_cast<OfficeStyle*>(mainindow->style()))
...
{
    return create_qtitan_className(QStringLiteral("RibbonStyle"));
}
...
void RibbonStyleDsgnPlugin::widgetManaged(QWidget* widget)
{
    ...
    if (QLatin1String(widget->metaObject()->className()) == create_qtitan_className(QStringLiteral("RibbonStyle")))
    {
        ...
        if (widgetStyle && QLatin1String(widgetStyle->metaObject()->className()) == create_qtitan_className(QStringLiteral("RibbonStyle")))
        {
            QMessageBox::critical(
                formWindow->mainContainer(),
...
{
    friend class RibbonStyleDsgnPlugin;
public:
...
bool DsgnFormWindowManagerFilter::verifyExistenceStyle(QWidget* style)
{
    ...
    if (QWidget* wd = container->widget(container->count()-1))
        return QLatin1String(wd->metaObject()->className()) == QLatin1String(style->metaObject()->className());
    return false;
}
            ...
            if (QDragEnterEvent* dragEvent = static_cast<QDragEnterEvent*>(event))
            {
                ...
                if (QMainWindow* mw = qobject_cast<QMainWindow*>(m_formWindow->mainContainer()))
                {
                    ...
                    if (QWidget* wd = mw->childAt(mw->mapFromGlobal(QCursor::pos())))
                        return QLatin1String(wd->metaObject()->className()) != QStringLiteral("QDesignerWidget");
                }
            }
        }
    }
