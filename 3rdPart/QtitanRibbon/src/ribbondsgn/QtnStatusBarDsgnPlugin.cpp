...
{
    if (QLatin1String(widget->metaObject()->className()) == create_qtitan_className(QStringLiteral("RibbonStatusBar")))
    {
        ...
        for (int i = 0; i < container->count(); ++i)
        {
            ...
            if (QLatin1String(w->metaObject()->className()) == create_qtitan_className(QStringLiteral("RibbonStatusBar")) ||
                QLatin1String(w->metaObject()->className()) == QStringLiteral("QStatusBar"))
            {
                QMessageBox::critical(
                    formWindow->mainContainer(),
                    tr("Can't add Ribbon StatusBar"), 
...
{
    return create_qtitan_className(QStringLiteral("RibbonStatusBar"));
}
...
{
    return QIcon(QStringLiteral(":res/statusbar_icon.png"));
}

QString DsgnRibbonStatusBarPlugin::toolTip() const
{
    return tr("StatusBar for Ribbon Interface");
}
...
{
    QString className = create_qtitan_className(QStringLiteral("RibbonStatusBar"));
    return QStringLiteral("<ui language=\"c++\">\n"
        " <widget class=\"" "%1" "\" name=\"statusBar\"/>\n"
        " <customwidgets>\n"
        "   <customwidget>\n"
        "       <class>" "%2" "</class>\n"
