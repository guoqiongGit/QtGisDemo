...
Office2007StylePrivate::Office2007StylePrivate()
    : m_themeType(Office2007Style::Blue)
...
/*! \internal */
void Office2007StylePrivate::initialization()
{
}
...
/*! \internal */
void Office2007StylePrivate::drawImage(const QPixmap& soSrc, QPainter& p, const QRect& rcDest, const QRect& rcSrc, QRect rcSizingMargins, QColor clrTransparent, bool alphaBlend) const
{
...
/*! \internal */
void Office2007StylePrivate::drawImage(const QPixmap& dcSrc, QPainter& p, const QRect& rcDest, const QRect& rcSrc, QRect rcSizingMargins, bool alphaBlend) const
{
...
void Office2007StylePrivate::drawMenuItemSeparator(const QStyleOption* opt, QPainter* painter, const QWidget* widget) const
{
    ...
    if (const QStyleOptionMenuItem* menuitem = qstyleoption_cast<const QStyleOptionMenuItem*>(opt))
    {
        ...
        else
        {
            ...
            painter->setPen(opt->palette.text().color());
            painter->drawText(rectText, text_flags, menuitem->text);
            painter->restore();
        }
    }
}
        ...
        if (m_themeType == Office2007Style::Black)
        {
            if (const QStyleOptionToolButton* toolbutton = qstyleoption_cast<const QStyleOptionToolButton *>(opt))
            {
                if (opt->state & QStyle::State_MouseOver)
                {
                    ...
                    tButton.palette.setColor(QPalette::ButtonText, toolbutton->palette.color(QPalette::HighlightedText));
...
bool Office2007StylePrivate::drawRibbonFrameGroups(const QStyleOption* opt, QPainter* painter, const QWidget* widget) const
{
    ...
    {
        if (optRibbonBar->pageColor != Qtitan::ContextColorNone)
        {
            QString resName(QStringLiteral("Image_RibbonFrameGroups"));
            resName += CommonStyle::qtn_getColorToString(optRibbonBar->pageColor);
            QPixmap image = loadPixmap(QStringLiteral("RibbonBar"), resName);
            if (!image.isNull())
                drawImage(image, *painter, opt->rect, sourceRectImage(image.rect(), 0, 1), image.width() < 5 ? QRect(QPoint(1, 8), QPoint(1, 8)) : QRect(QPoint(8, 8), QPoint(8, 8)));
        }
        else
            drawWidgetPixmap(QStringLiteral("RibbonBar"), QStringLiteral("Image_RibbonFrameGroups"), opt->rect, painter, 0, 1, QMargins(4, 4, 4, 4));
        return true;
    }
    return false;
}
...
{
    if (const RibbonGroupStyleOption* gr = qstyleoption_cast<const RibbonGroupStyleOption*>(opt))
    {
        QRect rcEntryCaption = opt->rect;
        rcEntryCaption.setTop(rcEntryCaption.bottom() - gr->heightCaption);
        rcEntryCaption.adjust(0, 0, 0, -2);
        QPixmap image, imageCaption;
        if (gr->pageColor == Qtitan::ContextColorNone)
        {
            if (w->property(_qtn_TitleGroupsVisible).toBool())
            {
        ...
        else
        {
            ...
            resName += CommonStyle::qtn_getColorToString(gr->pageColor);
        ...
        if (!imageCaption.isNull())
            rcEntry.setBottom(rcEntry.bottom() - gr->heightCaption);
...
//        drawPanelBackstageButton(opt, p, widget);
    ...
    if (widget && !widget->inherits(QTITAN_META_CLASSNAME("RibbonTitleButton")))
    {
        if (const QStyleOptionToolButton* toolbutton = qstyleoption_cast<const QStyleOptionToolButton*>(opt))
        {
...
//        qreal pixelRatio = painter->device()->devicePixelRatioF();
        QSize sz = QSize(CommonStylePrivate::dpiScaled(opt->iconSize.width(), w), CommonStylePrivate::dpiScaled(opt->iconSize.height(), w));
        rect.setSize(sz);
        drawImage(soRibbonOptionButton, *painter, rect, rcSrc, QRect(QPoint(3, 2), QPoint(2, 2)), QColor(0xFF, 0, 0xFF));
        return true;
    }
    return OfficeStylePrivate::drawRibbonOptionButton(option, painter, w);
}
...
bool Office2007StylePrivate::drawRibbonTab(const QStyleOption* opt, QPainter* painter, const QWidget* w) const
{
    ...
    if (pageColor != Qtitan::ContextColorNone)
    {
        ...
        if (selected || highlighted)
        {
            ...
            strImage += CommonStyle::qtn_getColorToString(pageColor);
            ...
            else
            {
                ...
                if (state == 4)
                {
                    ...
                    {
                        QPixmap pxImage = getCachedPixmap(QStringLiteral("RibbonTab"), strImage, opt->rect.size(), state, 6, QMargins(5, 5, 5, 5));
                        pxImage = pxImage.copy();
                        pxImage = pxImage.transformed(QTransform().rotate(180));
                        painter->drawPixmap(opt->rect, pxImage);
                    }
                    else
                        drawWidgetPixmap(QStringLiteral("RibbonTab"), strImage, opt->rect, painter, 5, 6, QMargins(5, 5, 5, 5));
                }
                else
                {
                    if (ribbonOption->tabBarPosition == 2)
                    {
                        QPixmap pxImage = getCachedPixmap(QStringLiteral("RibbonTab"), strImage, opt->rect.size(), state, 5, QMargins(5, 5, 5, 5));
                        pxImage = pxImage.copy();
                        pxImage = pxImage.transformed(QTransform().rotate(180));
                        painter->drawPixmap(opt->rect, pxImage);
                    }
                    else
                        drawWidgetPixmap(QStringLiteral("RibbonTab"), strImage, opt->rect, painter, state, 5, QMargins(5, 5, 5, 5));
                }
            }
        }
    }
    else
    {
        if (selected || highlighted)
        {
            ...
            if (ribbonOption->tabBarPosition == 2)
            {
                QPixmap pxImage = getCachedPixmap(QStringLiteral("RibbonTab"), image, opt->rect.size(), state, 6, QMargins(5, 5, 5, 5));
                pxImage = pxImage.copy();
                pxImage = pxImage.transformed(QTransform().rotate(180));
                painter->drawPixmap(opt->rect, pxImage);
            }
            else
                drawWidgetPixmap(QStringLiteral("RibbonTab"), image, opt->rect, painter, state, 6, QMargins(5, 5, 5, 5));
        }
    }
    ...
    if (w && w->inherits(QTITAN_META_CLASSNAME("RibbonTab")))
    {
        ...
        {
            QRect rcSeparator(QPoint(opt->rect.right() - 1, opt->rect.top()), QPoint(opt->rect.right() - 1, opt->rect.bottom() - 2));
            drawWidgetPixmap(QStringLiteral("RibbonTab"), QStringLiteral("Image_ContextTabSeparator"), rcSeparator, painter, 0, 1);
        }
    }
    return true;
...
{
    QTN_P(const Office2007Style);
    if (const RibbonGroupReducedStyleOption* optGroup = qstyleoption_cast<const RibbonGroupReducedStyleOption*>(opt))
    {
        ...
        if (pageColor != Qtitan::ContextColorNone)
        {
            ...
            resName += CommonStyle::qtn_getColorToString(optGroup->pageColor);
        ...
        // draw background
        drawImage(soImage, *painter, rcEntry, rcSrc, QRect(QPoint(5, 5), QPoint(5, 5)), (state == 1 && /*baseStyle()->isStyle2010()*/true));
...
bool Office2007StylePrivate::drawRibbonFileButton(const QStyleOptionComplex* opt, QPainter* painter, const QWidget* widget) const
{
    ...
    {
        if (optButton->toolButtonStyle != Qt::ToolButtonFollowStyle)
        {
            ...
            if (usePalette)
            {
                ...
                QBrush brHighlightButton = (resolve_mask & (1 << QPalette::Highlight)) ? optButton->palette.brush(QPalette::Active, QPalette::Highlight) : brButton;
                QBrush brPopupButton = (resolve_mask & (1 << QPalette::Dark)) ? optButton->palette.brush(QPalette::Active, QPalette::Dark) : brButton;
            ...
            QPalette pal = optButton->palette;
            pal.setBrush(QPalette::All, QPalette::ButtonText, Qt::white);
            p.proxy()->drawItemText(painter, optButton->rect, alignment, pal,
...
bool Office2007StylePrivate::drawRibbonTitleBarWidget(const QStyleOptionComplex* option, QPainter* painter, const QWidget* widget) const
{
    ...
    if (const TitleBarStyleOption* optTitleBar = qstyleoption_cast<const TitleBarStyleOption*>(option))
    {
        ...
        const bool isRibbonBar = widget && widget->parentWidget() && widget->parentWidget()->inherits(QTITAN_META_CLASSNAME("RibbonBar"));
        if (isRibbonBar)
            drawWidgetPixmap(QStringLiteral("RibbonTitleBarWidget"), QStringLiteral("Image_FrameTopCenter"), optTitleBar->rect.adjusted(0, -1, 0, 0), painter, 0, 2, QMargins(0, 5, 0, 3));

        if (optTitleBar->optButton.rect.isValid())
            p.proxy()->drawComplexControl(static_cast<QStyle::ComplexControl>(CommonStyle::CC_RibbonFileButton), &optTitleBar->optButton, painter, Q_NULL);
        ...
        for (QVector<TitleBarStyleOption::ContextData>::ConstIterator it = optTitleBar->listContextHeaders.begin(); it != optTitleBar->listContextHeaders.end(); ++it)
        {
            ...
            resName += CommonStyle::qtn_getColorToString(it->color);
        ...
        QPalette pal = optTitleBar->palette;
        ...
        p.proxy()->drawItemText(painter, optTitleBar->rcTitleText, optTitleBar->alignment, pal, true, text, QPalette::WindowText);
    }
    return true;
}
