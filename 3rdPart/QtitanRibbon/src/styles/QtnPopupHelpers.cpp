...
/*!
    \class StyleOptionPopupTitleBar
    \internal
*/
    ...
    switch (element) 
    {
        case PE_FrameWindow: 
            if (const QStyleOptionFrame* optFrame = qstyleoption_cast<const QStyleOptionFrame*>(option)) 
            {
                ...
                int titleBarHeight = proxy()->pixelMetric(PM_TitleBarHeight, Q_NULL, Q_NULL);
                ...
                StyleHelper::drawGradientFill(*p, rect, /*optFrame->palette*/m_palBackground.windowText().color().darker(111),
...
/*!
    \class PopupOffice2000DrawHelper
    \internal
*/
    ...
    switch (element) 
    {
        case PE_FrameWindow: 
            if (const QStyleOptionFrame* optFrame = qstyleoption_cast<const QStyleOptionFrame*>(option)) 
            {
                ...
                StyleHelper::drawGradientFill(*p, rect, m_palBackground.window().color(), m_palBackground.window().color(), true);
                StyleHelper::draw3DRect(*p, m_palFrame.shadow().color(), m_palFrame.shadow().color(),
                ...
                StyleHelper::drawGradientFill(*p, rcBackground, QColor(0, 0, 128), QColor(24, 180, 192), false);
                ...
                StyleHelper::drawGradientFill(*p, rcBackground, QColor(0, 0, 128), QColor(0, 0, 128), true);
            }
        break;
        default:
#if (QT_VERSION >= QT_VERSION_CHECK(5, 0, 0))
...
/*!
    \class PopupOffice2003DrawHelper
    \internal
*/
            ...
            case SC_TitleBarLabel: 
                {
                    if (!bar->text.isEmpty())
                    {
                        ...
                        QRect labelRect = bar->fontMetrics.boundingRect(bar->text);
                        int lineWidth = proxy()->pixelMetric(QStyle::PM_DefaultFrameWidth, Q_NULL, Q_NULL);
    ...
    switch (cc) 
    {
        case CC_TitleBar:
            if (const StyleOptionPopupTitleBar* tb = qstyleoption_cast<const StyleOptionPopupTitleBar*>(opt)) 
            {
                ...
                StyleHelper::drawGradientFill(*p, rcGripper, m_palGripper.color(QPalette::Light), m_palGripper.color(QPalette::Dark), true);
...
void PopupOffice2003DrawHelper::drawPrimitive(PrimitiveElement element, const QStyleOption* option, QPainter* p, const QWidget* widget) const
{
    switch (element) 
    {
        ...
        case PE_FrameWindow:
            ...
            {
                QRect rect = optFrame->rect;
                StyleHelper::drawGradientFill(*p, rect, m_palBackground.color(QPalette::Light), m_palBackground.color(QPalette::Dark), true);
                StyleHelper::draw3DRect(*p, m_palFrame.color(QPalette::Light), m_palFrame.color(QPalette::Dark), 
                    rect.x(), rect.y(), rect.width()-1, rect.height()-1, false);
            }
        break;
        default:
        ...
        case OS_SYSTEMWHITE:
            {
                ...
                m_clrButtonSelected = StyleHelper::colorMakePale(color.rgb(), .9);
                m_clrButtonPressed = StyleHelper::colorMakeDarker(m_clrButtonSelected.rgb(), .14);
...
/*!
    \class PopupOffice2007DrawHelper
    \internal
*/
...
/*!
    \class PopupOffice2010DrawHelper
    \internal
*/
...
*/
        case PE_FrameWindow: 
            if (const QStyleOptionFrame* optFrame = qstyleoption_cast<const QStyleOptionFrame*>(option)) 
            {
                ...
                StyleHelper::draw3DRect(*p, QColor(166, 180, 207), QColor(69, 86, 144), 
                    rect.x(), rect.y(), rect.width()-1, rect.height()-1, true);
                rect.adjust(1, 1, -1, -1);
                StyleHelper::draw3DRect(*p, QColor(255, 255, 255), QColor(207, 222, 244), 
                    rect.x(), rect.y(), rect.width()-1, rect.height()-1, true);
                rect.adjust(1, 1, -1, -1);
                ...
                StyleHelper::drawGradientFill(*p, rcBackground, QColor(207, 215, 236), QColor(255, 255, 255), true);

                rcBackground.setHeight(titleBarHeight + 1); 
                StyleHelper::drawGradientFill(*p, rcBackground, QColor(255, 255, 255), QColor(207, 221, 244), true);

                rcBackground.setHeight(40); 
                StyleHelper::drawGradientFill(*p, rcBackground, QColor(207, 221, 244), QColor(255, 255, 255), true);
...
//                rcBackground = CRect(rcClient.left, rcBackground.bottom, rcClient.right, rcClient.bottom);
                ...
                StyleHelper::draw3DRect(*p, QColor(114, 142, 184), QColor(185, 201, 239), 
