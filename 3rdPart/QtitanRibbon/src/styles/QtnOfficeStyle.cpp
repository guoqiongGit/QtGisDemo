...
void OfficeStylePrivate::initialization()
{
    ...
    QApplication::setFont(fontRibbon, "QMenu");
...
/*! \internal */
#if (QT_VERSION >= QT_VERSION_CHECK(5, 0, 0))
void OfficeStylePrivate::tabLayout(const QStyleOptionTab* opt, const QWidget* widget, QRect* textRect, QRect* iconRect) const
#else
void OfficeStylePrivate::tabLayout(const QStyleOptionTabV3* opt, const QWidget* widget, QRect* textRect, QRect* iconRect) const
#endif
{
    Q_ASSERT(textRect);
    Q_ASSERT(iconRect);
...
bool OfficeStylePrivate::drawTabBarTabLabel(const QStyleOption* opt, QPainter* painter, const QWidget* widget) const
{
...
#endif
    {
        ...
        p.proxy()->drawItemText(painter, tr, alignment, pal, tab->state & QStyle::State_Enabled, tab->text, QPalette::WindowText);
        if (verticalTabs)
            painter->restore();
/*
...
{
    if (widget != Q_NULL && widget->inherits(QTITAN_META_CLASSNAME("RibbonBar")))
    {
#ifdef Q_OS_WIN
        NONCLIENTMETRICS ncm;
        ncm.cbSize = FIELD_OFFSET(NONCLIENTMETRICS, lfMessageFont) + sizeof(LOGFONT);
        ::SystemParametersInfo(SPI_GETNONCLIENTMETRICS, ncm.cbSize, &ncm, 0);
        return qtn_LOGFONTtoQFont(ncm.lfMenuFont, true/*isDPIAware()*/, widget);
#else 
        QMenuBar menu;
        menu.setAttribute(Qt::WA_MacSmallSize);
        QFont fontRegular = QApplication::font(&menu);

        double scale = 1.0 / CommonStylePrivate::dpiScaled(1.0, widget);
