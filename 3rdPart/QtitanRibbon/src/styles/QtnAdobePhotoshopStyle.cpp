...
public:
    ...
    int m_heightMenuBar;
    AdobePhotoshopStyle::Theme m_themeType;
private:
...
AdobePhotoshopStylePrivate::AdobePhotoshopStylePrivate()
    : m_heightMenuBar(19)
    , m_themeType(AdobePhotoshopStyle::LightGray)
{
}
...
/*! \internal */
void AdobePhotoshopStylePrivate::initialization()
{
}
...
bool AdobePhotoshopStylePrivate::drawFrameWindow(const QStyleOption* option, QPainter* painter, const QWidget* widget) const
{
    if (const QStyleOptionFrame* frm = qstyleoption_cast<const QStyleOptionFrame *>(option))
    {
        ...
        else
        {
            ...
            painter->setPen(QPen(option->palette.midlight(), 3));
...
//        const int lineWidth = p.proxy()->pixelMetric(QStyle::PM_MdiSubWindowFrameWidth, option, widget);
        // bool active = (titleBar->titleBarState & QStyle::State_Active);
        QRect fullRect = titleBar->rect;
        QPalette palette = option->palette;
        QColor titleBarFrameBorder(palette.light().color());
        QColor titleBarHighlight(palette.midlight().color());
        QColor textColor(titleBar->palette.text().color());
        {
            // Fill title bar gradient
            QColor titlebarColor = QColor(palette.midlight().color());
            painter->fillRect(option->rect, titlebarColor);
...
*/
            ...
            !(titleBar->titleBarState& Qt::WindowMinimized))
        {
            ...
            if (minButtonRect.isValid())
            {
                ...
                optButton.palette.setBrush(QPalette::Light, brushLightButton);
                optButton.palette.setBrush(QPalette::Highlight, brushHighlightButton);
...
/*! \internal */
bool AdobePhotoshopStylePrivate::drawFrameFocusRect(const QStyleOption* option, QPainter* painter, const QWidget* w) const
{
    Q_UNUSED(option)
    Q_UNUSED(painter)
    Q_UNUSED(w)
...
bool AdobePhotoshopStylePrivate::drawRibbonFrameGallery(const QStyleOption* opt, QPainter* painter, const QWidget* widget) const
{
    if (const QtStyleOptionFrame* frameGallery = qstyleoption_cast<const QtStyleOptionFrame *>(opt))
    {
        ...
        if (frameGallery->features & QtStyleOptionFrame::Flat)
        {
            ...
            if (m_themeType == AdobePhotoshopStyle::LightGray)
            ...
            else if (m_themeType == AdobePhotoshopStyle::Gray)
            ...
            else if (m_themeType == AdobePhotoshopStyle::DarkGray)
            ...
            else if (m_themeType == AdobePhotoshopStyle::Black)
                painter->fillRect(opt->rect, selected && enabled ? StyleHelper::colorMakeLighter(opt->palette.color(QPalette::Base).rgb(), 0.80) :
                    StyleHelper::colorMakeLighter(opt->palette.color(QPalette::Base).rgb(), 0.6));
        }
...
bool AdobePhotoshopStylePrivate::drawPanelButtonCommand(const QStyleOption* option, QPainter* painter, const QWidget* widget) const
{
    ...
    else if (isEnabled)
    {
        if (widget != Q_NULL && widget->inherits("QDockWidgetTitleButton") && 
            (m_themeType == AdobePhotoshopStyle::DarkGray || m_themeType == AdobePhotoshopStyle::Black))
    ...
    const qreal radius = widget && widget->inherits("QPushButton") ? static_cast<qreal>(r.height()) / 2.0 : 2.0;
...
bool AdobePhotoshopStylePrivate::drawRibbonTab(const QStyleOption* opt, QPainter* painter, const QWidget* widget) const
{
    if (const RibbonOptionHeaderStyleOption* optTab = qstyleoption_cast<const RibbonOptionHeaderStyleOption*>(opt))
    {
        ...
        if ((optTab->state & QStyle::State_Selected) && !colorContext.isValid())
        {
            if (CommonStylePrivate::drawRibbonTab(opt, painter, widget))
            {
                QRect rectTab = optTab->rect.adjusted(1, 1, -1, -1);
                const int heightButton = CommonStylePrivate::dpiScaled(3, widget);
                rectTab.setHeight(heightButton);
                // fill the background of the tab
                painter->fillRect(rectTab, optTab->palette.color(QPalette::Highlight));
                return true;
            }
        }
    }
    ...
    if (const QStyleOptionHeader* header = qstyleoption_cast<const QStyleOptionHeader *>(option))
    {
        ...
        if (header->orientation == Qt::Horizontal)
        {
            if (header->state & QStyle::State_Sunken)
                painter->fillRect(rect.adjusted(0, 0, -2, -2), header->palette.button().color().dark(135));
            else if (header->state & QStyle::State_MouseOver)
                painter->fillRect(rect.adjusted(0, 0, -2, -2), header->palette.button().color().dark(120));
        }
        else
        {
            if (header->state & QStyle::State_Sunken)
                painter->fillRect(rect.adjusted(0, 0, -1, -1), header->palette.button().color().dark(135));
            else if (header->state & QStyle::State_MouseOver)
                painter->fillRect(rect.adjusted(0, 0, -1, -1), header->palette.button().color().dark(120));
        }
...
bool AdobePhotoshopStylePrivate::drawDockWidgetTitle(const QStyleOption* opt, QPainter* painter, const QWidget* widget) const
{
    ...
    else if (widget != Q_NULL && widget->inherits(QTITAN_META_CLASSNAME("DockTitleBar")))
    {
        if (const DockTitleBarStyleOption* dwOpt = qstyleoption_cast<const DockTitleBarStyleOption*>(opt))
        {
            ...
            if ((m_themeType == AdobePhotoshopStyle::LightGray || m_themeType == AdobePhotoshopStyle::DarkGray) && !dwOpt->lineRect.isEmpty())
            {
...
\brief Class implements Adobe Pthotoshop style for Qt application with four themes: light gray, gray, dark dray and black.
*/
/*!
...
QIcon AdobePhotoshopStyle::standardIcon(QStyle::StandardPixmap standardIcon, const QStyleOption* opt, const QWidget* widget) const
{
    ...
    if (widget != Q_NULL && widget->inherits("QDockWidget"))
    {
        bool black = (d.m_themeType == AdobePhotoshopStyle::DarkGray || d.m_themeType == AdobePhotoshopStyle::Black);
