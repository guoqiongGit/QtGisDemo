...
#include "QtnRibbonStatusBarPrivate.h"
...
\brief RibbonStatusBar class represents a horizontal bar at the bottom of the main window, adapted to display information about application state and to place UI controls on it.
*/

/*!
Constructs RibbonStatusBar object with the given \a parent.
*/
...
/*!
Destructor of the RibbonStatusBar object.
*/
...
void RibbonStatusBar::actionEvent(QActionEvent* event)
{
    ...
    if (event->type() == QEvent::ActionAdded)
    {
        ...
        int index = d.m_addPermanentAction ? (d.m_permanentWidgets.count() > 0 ? d.m_permanentWidgets.count() : -1) : 
            (d.m_widgets.count() > 0 ? d.m_widgets.count() : -1);
        if (event->before()) 
        {
            int ind = ::findIndexByAction(this, event->before());
            if (ind != -1)
