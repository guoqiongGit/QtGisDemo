...
public:
    ...
    OfficePopupWindow::PopupLocation m_popupLocation;
    OfficePopupWindow::PopupAnimation m_popupAnimation;
...
#ifdef Q_OS_WIN
    m_popupLocation = OfficePopupWindow::PopupLocationNearTaskBar;
#else // Q_OS_WIN
    m_popupLocation = OfficePopupWindow::PopupLocationBottomRight;
#endif // Q_OS_WIN
    m_popupAnimation = OfficePopupWindow::PopupAnimationNone;
...
void OfficePopupWindowPrivate::onCollapsing()
{
    ...
    if (m_popupAnimation == OfficePopupWindow::PopupAnimationNone || m_animationSpeed <= 0)
    {
        closePopup();
        return;
    }
    else if (m_popupAnimation == OfficePopupWindow::PopupAnimationFade)
    {
        m_stateTarget.transparency = 0;
    }
    else if (m_popupAnimation == OfficePopupWindow::PopupAnimationSlide)
    {
        QStyleOption opt;
        opt.init(&p);
    ...
    else if (m_popupAnimation == OfficePopupWindow::PopupAnimationUnfold)
    {
...
void OfficePopupWindowPrivate::onExpanding(bool updateCurrent)
{
    ...
    if (updateCurrent)
    {
        ...
        if (m_popupAnimation == OfficePopupWindow::PopupAnimationSlide)
...
void OfficePopupWindowPrivate::updateState(bool repaint)
{
    ...
    if (!m_capture)
        p.setWindowOpacity(static_cast<qreal>(m_stateCurrent.transparency/255.0));
    p.update();
}
...
void OfficePopupWindowPrivate::handleMouseMoveEvent(QMouseEvent* event)
{
    ...
    if (m_dragging && m_form)
    {
        ...
        QRect rc = p.rect();
        QRect rcScreenGeometry = qtn_availableGeometry(&p);
    ...
    if (m_popupState == PopupStateCollapsing)
    {
        ...
        if (m_popupAnimation == OfficePopupWindow::PopupAnimationFade)
            showPopup();
        else
            onExpanding(false);
    }
...
/*!
    ...
    \brief OfficePopupWindow class is a realization of a Microsoft Office popup window. Such window is widely used in products of Microsoft Office for showing messages or groups of messages.
    Typically, a window appears at the bottom right. Just above the desktop task bar.
    OfficeStyle and RibbonStyle have a property setPopupDecoration() to set the decoration of this window.
    With this property you can give a look of Microsoft Messanger (MSN) or it allows to decorate the window depending on the style which installed in QApplication.

    \raw HTML
        <table cellpadding="2" cellspacing="1" border="0" width="95%" class="indextable" align="center">
        <tr><td colspan=2><h2>Variants of the office popup windows with different decoration</h2><br><br></td></tr>
        <tr>
        <td><img src="images/ribbonui_officepopupwindow1.png"</td>
        <td><img src="images/ribbonui_officepopupwindow2.png"</td>
        </tr>
        <tr>
        <td><img src="images/ribbonui_officepopupwindow3.png"</td>
        <td><img src="images/ribbonui_officepopupwindow4.png"</td>
        </tr>
        </table>
    \endraw
*/
...
\value PopupLocationNearTaskBar OfficePopupWindow appear at the bottom-right corner of the desktop (above the desktop taskbar). Please note this value is relevant to OS Windows only.
\value PopupLocationBottomRight OfficePopupWindow appear in the bottom-right corner of the desktop.
\value PopupLocationCenter OfficePopupWindow appear in the center of the desktop.
*/

/*!
\enum OfficePopupWindow::PopupAnimation
This enum describes the style of how window will appearing on the screen.
\value PopupAnimationNone Animation is not using.
\value PopupAnimationFade Fade method of animation
\value PopupAnimationSlide Slide method of animation
\value PopupAnimationUnfold Unfold method of animation
*/

/*!
\fn void OfficePopupWindow::aboutToHide();
The signal is emitted just before the office popup window is hidden from the user.
\sa aboutToShow(), showPopup()
*/

/*!
\fn void OfficePopupWindow::aboutToShow();
The signal is emitted just before the office popup window is shown to the user.
\sa aboutToHide(), closePopup()
*/

/*
Constructs the OfficePopupWindow object with the given \a parent.
*/
...
/*!
Sets the popup window position on the desktop with given \a pos parameter.
*/ 
...
/*!
Sets the popup window animation type with given \a popupAnimation parameter.
*/ 
...
/*!
Sets the start \a transparency of the office popup window.
*/ 
...
/*!
Sets the display time for showing office popup window. Parameter \a time is the value in milliseconds.
*/ 
...
/*!
Sets the speed of the animation. \a time - the value of the speed  in milliseconds.
*/ 
...
/*!
Sets the drag-and-drop \a enabled flag for the popup box. If \a enabled is true then the user may use grip to drag-and-drop window over desktop.
*/ 
...
/*!
Sets the start \a location of the office popup window. 
*/ 
