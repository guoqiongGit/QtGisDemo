...
#define QTN_DIC_TAGNAME_RIBBON           QStringLiteral("RibbonStorage")
#define QTN_DIC_TAGNAME_RIBBON_QABAR     QStringLiteral("QuickAccessBar")
#define QTN_DIC_TAGNAME_RIBBON_BAR       QStringLiteral("RibbonBar")
#define QTN_DIC_TAGNAME_RIBBON_PAGE      QStringLiteral("RibbonPage")
#define QTN_DIC_TAGNAME_RIBBON_GROUP     QStringLiteral("RibbonGroup")
#define QTN_DIC_TAGNAME_RIBBON_ACTION    QStringLiteral("RibbonAction")

#define QTN_DIC_ATTRNAME_RIBBON_ID       QStringLiteral("nameId")
#define QTN_DIC_ATTRNAME_RIBBON_TITLE    QStringLiteral("title")
#define QTN_DIC_ATTRNAME_RIBBON_VISIBLE  QStringLiteral("visible")
#define QTN_DIC_ATTRNAME_RIBBON_TYPE     QStringLiteral("type")
...
{
    static QString prefix = QStringLiteral("__qtn_Action_%1");
...
{
    static QString prefix = QStringLiteral("__qtn_Group_%1");
...
{
    static QString prefix = QStringLiteral("__qtn_Page_%1");
...
void ToolBarCustomizeEngine::addDefaultToolBar(QToolBar* toolBar)
{
    ...
    {
        QAction* action = itAction.next();
        if (action->isVisible() && action->property(__qtn_Action_Invisible).toString() != QStringLiteral("__qtn_Action_Invisible") &&
            action->property(__qtn_Quick_Access_Button).toString() != QStringLiteral("__qtn_Quick_Access_Button"))
        {
            setActionId(action);
...
//        if (page->objectName() != QStringLiteral("__qtn_Page_Invisible"))
        {
            setPageId(page);
...
//        if (group->objectName() != QStringLiteral("__qtn_Group_Invisible") )
        {
            setGroupId(group);
...
void RibbonBarCustomizeEngine::setRibbonBar()
{
    ...
    else
    {
        ...
        while (itRegularPage.hasNext()) 
        {
            ...
            if (!m_regularPages.contains(currentPage))
            {
                if (currentPage->property(__qtn_Widget_Custom).toString() == QStringLiteral("__qtn_Page_Custom"))
...
//                        setGroupId(group);
                    ...
                    while (itRegularGroups.hasNext()) 
                    {
                        ...
                        if (!groupsPage.contains(regularGroup) && (regularGroup->property(__qtn_Widget_Custom).toString() == QStringLiteral("__qtn_Group_Custom") ||
                                                                   regularGroup->property(__qtn_Widget_Copy).toString() == QStringLiteral("__qtn_Group_Copy")) )
                        {
...
void RibbonBarCustomizeEngine::updateRibbonBar()
{
    ...
    if (currentIndex != -1)
    {
        if (RibbonPage* p = m_ribbonBar->page(currentIndex))
        {
            if (p->isVisible())
            {
                p->updateLayout();
            ...
            if (!m_regularHiddenPages.contains(p))
                return;
        }
    }
...
void RibbonBarCustomizeEngine::saveStateGroup(QXmlStreamWriter& stream, RibbonPage* page) const
{
    ...
    while (itGroup.hasNext()) 
    {
        if (RibbonGroup* group = itGroup.next()) 
        {
            ...
            bool custom = group->property(__qtn_Widget_Custom).toString() == QStringLiteral("__qtn_Group_Custom");
            bool copy = group->property(__qtn_Widget_Copy).toString() == QStringLiteral("__qtn_Group_Copy");
            ...
            if (custom)
                stream.writeAttribute(QTN_DIC_ATTRNAME_RIBBON_TYPE, QStringLiteral("CustomGroup"));
            else if (copy)
                stream.writeAttribute(QTN_DIC_ATTRNAME_RIBBON_TYPE, QStringLiteral("CopyGroup"));
...
void RibbonBarCustomizeEngine::saveStatePage(QXmlStreamWriter& stream) const
{
    ...
    while (itPage.hasNext()) 
    {
        if (RibbonPage* page = itPage.next()) 
        {
            ...
            if (page->property(__qtn_Widget_Custom).toString() == QStringLiteral("__qtn_Page_Custom"))
                stream.writeAttribute(QTN_DIC_ATTRNAME_RIBBON_TYPE, QStringLiteral("CustomPage"));
...
bool RibbonBarCustomizeEngine::restoreStatePage(QXmlStreamReader& stream)
{
    ...
    while (stream.tokenType() == QXmlStreamReader::StartElement)
    {
        ...
        else if (customPage)
        {
            ...
            newPage->setProperty(__qtn_Widget_Custom, QStringLiteral("__qtn_Page_Custom"));
...
void RibbonCustomizeManager::deleteGroup(RibbonPage* page, int index)
{
    ...
    QList<RibbonGroup*> lstGroups = d.m_ribbonManager->defaultPageGroups().value(page);
    const bool isExist = lstGroups.contains(group);

    if (!isExist && group->parent() == Q_NULL)
        delete group;
}
...
bool RibbonCustomizeManager::saveStateToDevice(QIODevice* device)
{
    ...
    xmlwriter.writeNamespace(QStringLiteral("http://www.devmachines.com/qt/qtitan"), QStringLiteral("Qtitan"));
    xmlwriter.writeStartElement(QTN_DIC_TAGNAME_RIBBON);
    xmlwriter.writeAttribute(QStringLiteral("type"), QStringLiteral("customize"));
