...
/*!
\class RibbonControlSizeDefinition
...
void RibbonToolBarControlPrivate::recalcWidths(int height)
{
    ...
    if (parentGroup == Q_NULL || parentGroup->isHidden())
        return;
    ...
    opt.init(parentGroup);
    opt.text = parentGroup->title();

    p.setRowsCount(2);
    m_isCalcWidth = true;

    QRect rect = qtn_availableGeometry(&p);
    QRect rectScreen(QPoint(0, 0), rect.size());

    bool m_floatyMode = false;
    const int maxRows = 3;
    const int widthDefaultButton = parentGroup->style()->pixelMetric(static_cast<QStyle::PixelMetric>(CommonStyle::PM_RibbonReducedGroupWidth), &opt, parentGroup);
    int lastWidth = -1;
    int dx = 16;
...
void RibbonToolBarControlPrivate::showDefaultButton()
{
    ...
    opt.init(parentGroup);
    opt.text = parentGroup->title();
    const int widthDefaultButton = p.parentGroup()->style()->pixelMetric((QStyle::PixelMetric)CommonStyle::PM_RibbonReducedGroupWidth, &opt, parentGroup);
    ...
    for (RibbonToolBarControlPrivate::RibbonToolBarActionGroupItems::const_iterator it = group.constBegin(); it != group.constEnd(); ++it)
    {
        if (it->action->isSeparator())
        {
            if (it->action->isVisible())
            {
                QStyleOption opt;
                opt.init(&p);
                const int widthSeparator = parentGroup->style()->pixelMetric(QStyle::PM_ToolBarSeparatorExtent, &opt, &p);
...
{
    QTN_D(RibbonToolBarControl)
    bool result = RibbonControl::event(event);
    if (d.m_dirty && (event->type() == QEvent::LayoutRequest || event->type() == QEvent::Show))
    {
