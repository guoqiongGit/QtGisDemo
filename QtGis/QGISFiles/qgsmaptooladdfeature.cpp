...
QgsMapToolAddFeature::QgsMapToolAddFeature( QgsMapCanvas *canvas, CaptureMode mode )
  : QgsMapToolDigitizeFeature( canvas, new QgsAdvancedDigitizingDockWidget(canvas), mode )
...
bool QgsMapToolAddFeature::addFeature( QgsVectorLayer *vlayer, const QgsFeature &f, bool showModal )
{
  ...
  //  delete action;
  //return res;
    //create feature without attributes table
    QgsFeature feat(f);
    bool res = vlayer->addFeature(feat);
    return res;
}
